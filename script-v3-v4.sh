...
docker_build() {
    ...
    build_pkg "$image" linux-cachyos "${build_env[@]}" -e "_features=cachy o3" -e "_cpusched=cachyos" -e _package_suffix=gcc
    build_pkg "$image" linux-cachyos "${build_env[@]}" -e "_features=cachy o3" -e "_cpusched=eevdf" -e _package_suffix=lto
    build_pkg "$image" linux-cachyos "${build_env[@]}" -e "_features=cachy o3" -e "_cpusched=bore" -e _package_suffix=lto
    build_pkg "$image" linux-cachyos "${build_env[@]}" -e "_features=cachy o3" -e "_cpusched=bmq" -e _package_suffix=lto
    build_pkg "$image" linux-cachyos "${build_env[@]}" -e "_features=cachy o3 rt" -e "_cpusched=bore" -e _package_suffix=lto
    build_pkg "$image" linux-cachyos "${build_env[@]}" -e "_features=server o3" -e "_cpusched=cachyos" -e _package_suffix=lto
    build_pkg "$image" linux-cachyos "${build_env[@]}" -e "_features=cachy o3 handheld" -e "_cpusched=bore" -e _package_suffix=lto
}
